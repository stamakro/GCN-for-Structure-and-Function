...
class GraphDataset(Dataset):
    def __init__(self, names_file, terms_file, feats_dir, feats_type='embeddings', edges_type='normal'):
        # Initialize data
        ...
        with open(terms_file, 'rb') as fin:
            terms_mat = pickle.load(fin)
        self.terms = {name: terms_mat[i] for i, name in enumerate(self.names)}
    ...
    def __getitem__(self, index):
        ...
        # Load pickle file with dictionary containing embeddings (LxF), edge_indexes
        ...
        # Get labels (1xN)
        labels = np.expand_dims(self.terms[name], 0).astype(np.float32)
...
class CNN1DDataset(Dataset):
    def __init__(self, names_file, terms_file, feats_dir, feats_type='embeddings'):
        # Initialize data
        self.names = list(np.loadtxt(names_file, dtype=str))
        self.feats_dir = feats_dir
        self.feats_type = feats_type
    ...
    def __getitem__(self, index):
        ...
        # Load pickle file with dictionary containing embeddings (LxF), sequence (L)
        d = pickle.load(open(self.feats_dir + '/' + name + '.pkl', 'rb'))
        seq = d['sequence']
        seqlen = len(seq)
        ...
        # Get labels (N)
        labels = self.terms[name].astype(np.float32)
...
class CNN2DDataset(Dataset):
    def __init__(self, data_file, terms_file, feats_dir):
        # Initialize data
    ...
    def get_one(self, name):
        # Load pickle file with dictionary containing sequence (L)
        d = pickle.load(open(self.feats_dir + '/' + name + '.pkl', 'rb'))
...
class CNN1D2DDataset(Dataset):
    def __init__(self, data_file, terms_file, feats_dir, feats_type='embeddings'):
        # Initialize data
        data = np.loadtxt(data_file, dtype=str)
        try:
            self.names = data[:,0]
...
class MLPDataset(Dataset):
    def __init__(self, names_file, terms_file, feats_dir, feats_type='embeddings', kmer_ncomp=-1):
        # Initialize data
        self.names = list(np.loadtxt(names_file, dtype=str))
        self.feats_dir = feats_dir
        self.feats_type = feats_type
        self.kmer_ncomp = kmer_ncomp
    ...
    def __getitem__(self, index):
        ...
        if self.kmer_ncomp != -1:
            features = features[:self.kmer_ncomp]

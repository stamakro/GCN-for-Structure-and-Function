...
from datasets_cv import GraphDataset, CNN1DDataset, CNN2DDataset, CNN1D2DDataset, MLPDataset
...
parser.add_argument('--train_terms',    dest='train_terms',     default='data/Ytrain.pkl')
parser.add_argument('--valid_file',     dest='valid_file',      default='data/valid.names')
parser.add_argument('--valid_terms',    dest='valid_terms',     default='data/Yvalidation.pkl')
...
parser.add_argument('--test_terms',     dest='test_terms',      default='data/Ytest.pkl')
...
if args.phase == 'train':
    ...
    if args.net_type == 'gcn' or args.net_type == 'chebcn' or args.net_type == 'gmmcn' or args.net_type == 'gincn':
        train_set = GraphDataset(args.train_file, args.train_terms, args.feats_dir, args.feats_type, args.edges_type)
        ...
        valid_set = GraphDataset(args.valid_file, args.valid_terms, args.feats_dir, args.feats_type, args.edges_type)
        valid_loader = pygeoDataLoader(valid_set, batch_size=1, shuffle=False)

    elif args.net_type == 'cnn1d' or args.net_type == 'deepgoplus':
        train_set = CNN1DDataset(args.train_file, args.train_terms, args.feats_dir, args.feats_type)
        ...
        valid_set = CNN1DDataset(args.valid_file, args.valid_terms, args.feats_dir, args.feats_type)
        valid_loader = pyDataLoader(valid_set, batch_size=1, shuffle=False, collate_fn=cnn1d_collate)

    elif args.net_type == 'cnn2d':
        train_set = CNN2DDataset(args.train_file, args.train_terms, args.feats_dir)
        ...
        valid_set = CNN2DDataset(args.valid_file, args.valid_terms, args.feats_dir)
        valid_loader = pyDataLoader(valid_set, batch_size=1, shuffle=False, collate_fn=cnn2d_collate)

    elif args.net_type == 'cnn1d2d':
        train_set = CNN1D2DDataset(args.train_file, args.train_terms, args.feats_dir, args.feats_type)
        train_sampler = VariableBatchSizeSampler(train_set, shuffle=True, drop_last=False)
        ...
        valid_set = CNN1D2DDataset(args.valid_file, args.valid_terms, args.feats_dir, args.feats_type)
        valid_loader = pyDataLoader(valid_set, batch_size=1, shuffle=False, collate_fn=cnn1d2d_collate)

    elif args.net_type == 'mlp':
        train_set = MLPDataset(args.train_file, args.train_terms, args.feats_dir, args.feats_type, args.input_dim)
        ...
        valid_set = MLPDataset(args.valid_file, args.valid_terms, args.feats_dir, args.feats_type, args.input_dim)
    ...
    train(device=device, net=net, criterion=criterion,
          ...
          icvec=icvec, ckpt_dir=ckpt_dir, logs_dir=logs_dir, evaluate_train=True)
...
# Independent test phase
    ...
    if args.net_type == 'gcn' or args.net_type == 'chebcn' or args.net_type == 'gmmcn' or args.net_type == 'gincn':
        test_set = GraphDataset(args.test_file, args.test_terms, args.feats_dir, args.feats_type, args.edges_type)
        test_loader = pygeoDataLoader(test_set, batch_size=1, shuffle=False)

    elif args.net_type == 'cnn1d' or args.net_type == 'deepgoplus':
        test_set = CNN1DDataset(args.test_file, args.test_terms, args.feats_dir, args.feats_type)
        test_loader = pyDataLoader(test_set, batch_size=1, shuffle=False, collate_fn=cnn1d_collate)

    elif args.net_type == 'cnn2d':
        test_set = CNN2DDataset(args.test_file, args.test_terms, args.feats_dir)
        test_loader = pyDataLoader(test_set, batch_size=1, shuffle=False, collate_fn=cnn2d_collate)

    elif args.net_type == 'cnn1d2d':
        test_set = CNN1D2DDataset(args.test_file, args.test_terms, args.feats_dir, args.feats_type)
        test_loader = pyDataLoader(test_set, batch_size=1, shuffle=False, collate_fn=cnn1d2d_collate)

    elif args.net_type == 'mlp':
        test_set = MLPDataset(args.test_file, args.test_terms, args.feats_dir, args.feats_type, args.input_dim)
